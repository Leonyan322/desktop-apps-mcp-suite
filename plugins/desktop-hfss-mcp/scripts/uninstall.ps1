...
$expected_root = [IO.Path]::GetFullPath((Join-Path $engineering_root "hfss"))
...
if (Test-Path -LiteralPath $actual_root) {
    if ($PSCmdlet.ShouldProcess($actual_root, "Remove only the HFSS MCP runtime")) {
        Remove-Item -LiteralPath $actual_root -Recurse -Force
        Write-Host "Removed the HFSS MCP runtime: $actual_root"
        Write-Host "This removal is not recoverable by this script. HFSS user projects were not deleted."
    }
} else {
    Write-Host "The HFSS MCP runtime does not exist: $actual_root"
}
