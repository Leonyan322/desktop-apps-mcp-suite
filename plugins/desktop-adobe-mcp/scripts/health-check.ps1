...
function Read-McpResponse {
    ...
    while ([datetime]::UtcNow -lt $deadline) {
        ...
        try {
            ...
            $line_summary = if ($line.Length -gt 160) { $line.Substring(0, 160) + "..." } else { $line }
...
function Invoke-McpStdioProbe {
    ...
    try {
        ...
        $initialize_request = @{
            ...
            params = @{
                ...
                capabilities = @{}
                clientInfo = @{
                    name = "desktop-adobe-mcp-health-check"
                    version = "0.1.0"
                }
            }
        ...
        if ($process_started -and $process.HasExited -eq $false) {
    ...
    if ($null -ne $probe_error) {
        ...
        throw "$ServerName MCP stdio 探针失败：$detail"
    }
...
try {
    Assert-GitCommit `
        -RepositoryPath $illustrator_runtime `
        -ExpectedCommit $script:illustrator_commit `
        -ExpectedRemote $script:illustrator_repo_url
    Write-Output "[通过] Illustrator MCP 提交：$script:illustrator_commit"
}
catch {
    $failures.Add($_.Exception.Message)
}

$venv_python = Join-Path $illustrator_runtime ".venv\Scripts\python.exe"
if (-not (Test-Path -LiteralPath $venv_python -PathType Leaf)) {
    $failures.Add("缺少 Illustrator Python 虚拟环境：$venv_python")
}
else {
    & $venv_python -c "import mcp, PIL, win32com.client, illustrator; print('Illustrator Python dependencies OK')"
    if ($LASTEXITCODE -ne 0) {
        $failures.Add("Illustrator Python 依赖导入失败。")
    }
    else {
        Write-Output "[通过] Illustrator Python 依赖"
    }
}
...
if (-not $illustrator_info.registered) {
    ...
    if ($illustrator_info.cur_ver -ne "Illustrator.Application.24") {
        $warnings.Add("Illustrator CurVer 为 $($illustrator_info.cur_ver)，不是 2020 对应的 .24。")
    }
}

if ($failures.Count -eq 0) {
    try {
        $photoshop_tool_count = Invoke-McpStdioProbe `
            -ServerName "Photoshop" `
            -StartScript (Join-Path $PSScriptRoot "start-photoshop.ps1")
        Write-Output "[通过] Photoshop MCP initialize/tools/list：$photoshop_tool_count 个工具"
    }
    catch {
        $failures.Add($_.Exception.Message)
    }
    ...
    try {
        $illustrator_tool_count = Invoke-McpStdioProbe `
            -ServerName "Illustrator" `
            -StartScript (Join-Path $PSScriptRoot "start-illustrator.ps1")
        Write-Output "[通过] Illustrator MCP initialize/tools/list：$illustrator_tool_count 个工具"
    }
    catch {
        $failures.Add($_.Exception.Message)
    }
...
if ($failures.Count -gt 0) {
    ...
    throw "Adobe MCP 健康检查失败，共 $($failures.Count) 项。"
}

Write-Output "Adobe MCP 健康检查通过；未执行写操作、任意脚本或屏幕截图。"
