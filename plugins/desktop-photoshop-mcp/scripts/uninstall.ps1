...
$runtime_path = [System.IO.Path]::GetFullPath((Get-PhotoshopRuntimePath)).TrimEnd('\')
$local_app_data = [System.IO.Path]::GetFullPath(
    [System.Environment]::GetFolderPath([System.Environment+SpecialFolder]::LocalApplicationData)
).TrimEnd('\')
$expected_path = [System.IO.Path]::GetFullPath(
    (Join-Path $local_app_data "desktop-apps-mcp-suite\adobe\photoshop-mcp")
).TrimEnd('\')
...
if ((Split-Path -Leaf $runtime_path) -ne "photoshop-mcp") {
    throw "运行时目录名称异常，拒绝删除：$runtime_path"
}

if (-not (Test-Path -LiteralPath $runtime_path)) {
    Write-Output "Photoshop MCP 运行时不存在，无需卸载。"
    exit 0
}
...
if ($PSCmdlet.ShouldProcess($runtime_path, "删除 Photoshop MCP 用户级运行时")) {
    Remove-Item -LiteralPath $runtime_path -Recurse -Force
    Write-Output "已删除：$runtime_path"
    Write-Output "该运行时目录未移入回收站；Illustrator 运行时、插件文件和 Adobe 文档未删除。"
}
