...
foreach ($com_check in $com_checks) {
    ...
    Add-CheckResult -Name $com_check.Name -Passed $registered -Details $(if ($registered) { $com_check.ProgId } else { "未注册 $($com_check.ProgId)。" })
}

try {
    $runtime_root = Get-OfficeRuntimeRoot
    Add-CheckResult -Name '运行时目录' -Passed $true -Details $runtime_root
}
catch {
...
if ($results.Status -contains 'FAIL') {
    Write-Error '预检未通过。请安装缺少的软件并先手动完成 Office 首次启动或激活。'
    exit 1
}

Write-Host '预检通过。Office 首次启动、激活、受保护视图或文件恢复弹窗仍需用户在应用界面处理。'
