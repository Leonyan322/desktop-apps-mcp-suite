...
$expected_parent = [System.IO.Path]::GetFullPath(
    (Join-Path $local_app_data 'desktop-apps-mcp-suite')
)
$resolved_parent = [System.IO.Path]::GetFullPath(
    (Split-Path -Parent $runtime_root)
)

if ($resolved_parent -ne $expected_parent -or (Split-Path -Leaf $runtime_root) -ne 'office') {
...
if (Test-Path -LiteralPath $runtime_root) {
    ...
    Write-Host "已删除 Office MCP 运行时：$runtime_root"
}
else {
    Write-Host "Office MCP 运行时不存在：$runtime_root"
}

Write-Host '未删除任何 Word、Excel 或 PowerPoint 用户文档，也未关闭 Office 应用。'
