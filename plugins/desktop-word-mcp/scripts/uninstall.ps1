...
if ($resolved_parent -ne $expected_parent -or (Split-Path -Leaf $runtime_root) -ne 'officemcp') {
...
if (Test-Path -LiteralPath $runtime_root) {
    ...
    if ($PSCmdlet.ShouldProcess($runtime_root, '删除 Word MCP 运行时')) {
        Remove-Item -LiteralPath $runtime_root -Recurse -Force
        Write-Host "已删除 Word MCP 运行时：$runtime_root"
    }
}
else {
    Write-Host "Word MCP 运行时不存在：$runtime_root"
}

Write-Host '未删除任何 Word 文档，也未关闭 Word。'
