...
Add-CheckResult -Name 'Word COM' -Passed $registered -Details $(if ($registered) { 'Word.Application' } else { '未注册 Word.Application。' })

try {
    Add-CheckResult -Name '运行时目录' -Passed $true -Details (Get-WordRuntimeRoot)
}
catch {
...
if ($results.Status -contains 'FAIL') {
    Write-Error '预检未通过。请安装缺少的软件并先手动完成 Word 首次启动或激活。'
    exit 1
}

Write-Host '预检通过。Word 首次启动、激活、受保护视图或文件恢复弹窗仍需用户在应用界面处理。'
