...
function Invoke-McpStdioProbe {
    <#
    .SYNOPSIS
    初始化 Illustrator MCP 并确认 tools/list 非空。
    ...
    try {
        ...
        $initialize_request = @{
            ...
            params = @{
                ...
                capabilities = @{}
                clientInfo = @{
                    name = "desktop-illustrator-mcp-health-check"
                    version = "0.2.0"
                }
            }
    ...
    if ($null -ne $probe_error) {
        ...
        throw "Illustrator MCP stdio 探针失败：$detail"
    }
...
if (-not (Test-Path -LiteralPath $venv_python -PathType Leaf)) {
    ...
    if ($LASTEXITCODE -ne 0) {
        $failures.Add("Illustrator Python 依赖或本地包导入失败。")
    }
    else {
        Write-Output "[通过] Illustrator Python 依赖与本地包"
    }
}
...
if ($failures.Count -eq 0) {
    try {
        ...
        Write-Output "[通过] Illustrator MCP initialize/tools/list：$tool_count 个工具"
    }
    catch {
        $failures.Add($_.Exception.Message)
    }
...
if ($failures.Count -gt 0) {
    ...
    throw "Illustrator MCP 健康检查失败，共 $($failures.Count) 项。"
}

Write-Output "Illustrator MCP 健康检查通过；未执行写操作、任意脚本或屏幕截图。"
