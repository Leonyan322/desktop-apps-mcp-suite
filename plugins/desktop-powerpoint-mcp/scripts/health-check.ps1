...
$runtime_python = Join-Path (Get-PowerPointRuntimeRoot) '.venv\Scripts\python.exe'
...
Add-HealthResult -Name 'PowerPoint MCP' -Passed $package_result.Passed -Details $package_result.Details

$registered = Test-ComRegistration -ProgId 'PowerPoint.Application'
Add-HealthResult -Name 'PowerPoint COM' -Passed $registered -Details $(if ($registered) { 'PowerPoint.Application' } else { '未注册。' })

$probe_python = Find-Python312
if ($null -eq $probe_python) {
    Add-HealthResult -Name 'MCP ppt' -Passed $false -Details '未找到 Python 3.12。'
}
else {
    ...
    $probe_arguments = @($probe_python.PrefixArguments) + @(
        ...
        '--name', 'ppt', '--timeout', '45', '--cwd', $plugin_root, '--',
        'powershell.exe', '-NoLogo', '-NoProfile', '-NonInteractive',
        '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $PSScriptRoot 'start-ppt.ps1')
    )
    ...
    Add-HealthResult -Name 'MCP ppt' -Passed ($probe_exit_code -eq 0) -Details $probe_output
}

$results | Format-Table -AutoSize
if ($results.Status -contains 'FAIL') {
    Write-Error 'PowerPoint MCP 健康检查未通过。可重新运行 install.ps1 修复运行时。'
    exit 1
}

Write-Host '健康检查通过。只执行了 MCP initialize 与 tools/list，未调用演示文稿业务工具。'
